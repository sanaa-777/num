        ...
        <div class="bg-white rounded-xl p-4 md:p-6 border border-gray-100">
          <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div><label class="block text-xs font-medium text-gray-700 mb-1">القسم الرئيسي *</label><select id="placeCategory" onchange="App.updateSubs()" class="w-full py-2.5 text-xs rounded-lg border border-gray-200 bg-white"><option value="">اختر القسم</option>${Data.categories.map(c => `<option value="${c.id}">${c.name}</option>`).join('')}</select></div>
              <div><label class="block text-xs font-medium text-gray-700 mb-1">القسم الفرعي *</label><select id="placeSubCategory" class="w-full py-2.5 text-xs rounded-lg border border-gray-200 bg-white"><option value="">اختر القسم الفرعي</option></select></div>
            </div>
            <div><label class="block text-xs font-medium text-gray-700 mb-1">المدينة *</label><select id="placeCity" class="w-full py-2.5 text-xs rounded-lg border border-gray-200 bg-white"><option value="">اختر المدينة</option>${Data.cities.map(c => `<option value="${c.id}">${c.name}</option>`).join('')}</select></div>
